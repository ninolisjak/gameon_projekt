...
function detectCardType(number: string): 'visa' | 'mastercard' | 'other' {
  ...
  const p2 = parseInt(d.slice(0, 2));
  const p4 = parseInt(d.slice(0, 4));
  if ((p2 >= 51 && p2 <= 55) || (p4 >= 2221 && p4 <= 2720)) return 'mastercard';
  return 'other';
}

function isExpiryFormatValid(expiry: string): boolean {
...
export default function PaymentCardModal({
  visible, amount, entityType, entityId, userId, description, onClose, onSuccess,
}: Props) {
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          ...
          >
            ...
            <View style={{
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <View style={{
                  ...
                  backgroundColor: '#e8c84a', borderWidth: 1, borderColor: '#c9a830', overflow: 'hidden',
                }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 18 }}>
                <View>
                  <Text style={{ color: 'rgba(255,255,255,0.55)', fontSize: 9, letterSpacing: 1, textTransform: 'uppercase' }}>Ime imetnika</Text>
                  <Text style={{ color: '#fff', fontSize: 13, fontWeight: '700', marginTop: 3, letterSpacing: 0.5 }}>
                    {name.toUpperCase() || '·····  ·····'}
                  </Text>
                </View>
                <View style={{ alignItems: 'flex-end' }}>
                  <Text style={{ color: 'rgba(255,255,255,0.55)', fontSize: 9, letterSpacing: 1, textTransform: 'uppercase' }}>Veljavnost</Text>
            ...
            <View style={{
              ...
              <View style={{ flexDirection: 'row', gap: 12 }}>
                <View style={{ flex: 1 }}>
                  ...
                  <View style={{
                    backgroundColor: colors.bg, borderRadius: 12,
                    borderWidth: 1, borderColor: colors.border, paddingHorizontal: 14, height: 52, justifyContent: 'center',
                  }}>
                    <TextInput
                      value={expiry}
            ...
            >
              {processing ? (
                ...
                <>
                  ...
                  <Text style={{ color: isFormValid ? '#fff' : colors.textFaint, fontSize: 16, fontWeight: '900' }}>
                    Plačaj {fmt(amount)} €
                  </Text>
                </>
              )}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              ...
              <Text style={{ color: colors.textFaint, fontSize: 11 }}>Plačilo je šifrirano in varno</Text>
            </View>
          </ScrollView>
